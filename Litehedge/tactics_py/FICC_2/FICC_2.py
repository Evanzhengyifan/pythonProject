...
def run(d: list, t: str, r: int, h: int, ts: str):
    ...
    def handle_bar(context, bar_dict):
        # 开始编写你的主要的算法逻辑
        hold_list = [i.order_book_id for i in get_positions()]
        for i in set(hold_list+list(context.target_long.keys())+list(context.target_short.keys())):
            # 实际持仓
            buy_quantity = context.portfolio.positions[i].buy_quantity
            sell_quantity = context.portfolio.positions[i].sell_quantity
            if i in context.target_long:
                if i in hold_list:
                    if sell_quantity:
                        order_to(i, 0)
                    if buy_quantity != context.target_long[i]['amount']:
                ...
                if i in hold_list:
                    if buy_quantity:
                        order_to(i, 0)
                    if sell_quantity != abs(context.target_short[i]['amount']):
    ...
    def main(context):
        ...
        print(get_previous_trading_date(date=context.now, n=20), context.now)
    ...
    def cal_roll(context, book_id: list):
        ...
        # 获得合约对应的品种，并合并一起
        df['underlying_symbol'] = df['order_book_id'].str.slice(0, -3)
        if context.TS == 'TS1':
            # 'TS1'为近月和次近月
            # 得到近月合约
            ...
            # 得到近月合约
            df['near_book_id'] = df['underlying_symbol'].map(lambda x: rq.futures.get_contracts(x, context.now)[0])
            df['far_book_id'] = df['underlying_symbol'].map(lambda x: rq.futures.get_dominant(x, start_date=context.now, end_date=context.now).item())
            df = cal(context, df)
        elif context.TS == 'TS3':
            # 'TS3'为近月和最远月
            # 得到近月合约
            ...
            # 得到主力合约
            df['book_id'] = df['underlying_symbol'].map(lambda x: rq.futures.get_dominant(x, start_date=context.now, end_date=context.now).item())
            # 得到到期日数据
            df['order_days'] = list(map(lambda x: x.days_to_expire(context.now), rq.instruments(df['book_id'].tolist())))
            ...
            df['near_book_id'] = np.where(df['order_days'] < df['sub_days'], df['book_id'], df['sub_book_id'])
            df['far_book_id'] = np.where(df['order_days'] < df['sub_days'], df['sub_book_id'], df['book_id'])
            df = cal(context, df)
        # 计算展期收益率
...
result = run(DATE_TIME, T[1], R[2], H[2], TS[0])
